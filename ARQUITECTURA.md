...
**Hoja de ruta (procesamiento por regiones):** detección previa de layout con un modelo ligero (YOLOv8: cajetín, BOM, notas), extracción de cada caja como sub-imagen a resolución nativa, y OCR tradicional (Tesseract/Azure OCR) restringido a la región, usando el VLM solo para dar contexto estructural. Igual que el MICL visual, exige binarios/modelos adicionales; queda documentado como extensión natural de `server/extract.js` (un paso previo que convierta `dataBase64` en una lista de parches).
