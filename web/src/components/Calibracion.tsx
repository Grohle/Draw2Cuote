...
export function Calibracion({ onCerrar, t }: Props) {
  ...
  const conDatos = (datos?.campos ?? []).filter((cc) => cc.vecesVisto > 0);
  ...
  return (
    <div className="modal-fondo" onClick={onCerrar}>
      <div className="modal modal--ancho" role="dialog" aria-label={t.app.tituloPrecision} onClick={(e) => e.stopPropagation()}>
        ...
        {datos && datos.totalAnalisisConFeedback === 0 && <p className="modal__nota">{c.sinDatos}</p>}

        {datos && datos.totalAnalisisConFeedback > 0 && (
          <>
            <p className="modal__nota">{c.totalAnalisis(datos.totalAnalisisConFeedback)}</p>
